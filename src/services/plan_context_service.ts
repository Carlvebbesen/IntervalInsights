...
export async function loadPlanGuardContext(
  ...
  const loadPlanSettings = async (): Promise<{
    settings: Record<string, unknown>;
    raceEventId: number | null;
  }> => {
    try {
      const row = await planRepo.findMetaForUser(db, userId, planId);
      return {
        settings: asRecord(asRecord(row?.meta).settings),
        raceEventId: row?.raceEventId ?? null,
      };
    } catch (err) {
      log.warn({ err }, "plan settings load failed — falling back to defaults");
      return { settings: {}, raceEventId: null };
    }
  };
  ...
  const loadBaseline = async (): Promise<{
    ...
    try {
      ...
      return { baselineWeeklyMeters: null, longestRunMeters: null };
    }
  };
  ...
  const [{ settings, raceEventId }, { baselineWeeklyMeters, longestRunMeters }, activeInjuries] =
    await Promise.all([loadPlanSettings(), loadBaseline(), loadActiveInjuries()]);
