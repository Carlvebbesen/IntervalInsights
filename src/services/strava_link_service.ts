...
// Hard cap so a single synchronous request stays bounded in wall-clock time.
  ...
  for (let page = 1; page <= MAX_LIST_PAGES; page++) {
    ...
    for (const summary of summaries) {
      ...
      try {
        ...
        result.failed++;
        logger.error({ err, stravaActivityId: summary.id }, "Strava master sync failed for activity");
      }
