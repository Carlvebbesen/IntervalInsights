...
- **Tools:** `src/mcp/server.ts` reuses the coach `registry` via `runTool`. `selectMcpTools` filters on `llmBacked` and `requires` only (`db` always; `strava`/`intervals` only when linked) — there is **no read/write distinction**, so every mutation tool in the registry (`create_training_plan`, `add_plan_week`, `add_planned_session`, `update_planned_session`, `apply_plan_revision`, the race-event writers, …) is live to external MCP clients by design. Tools that make their own server-side OpenAI calls (`OPENAI_BACKED_TOOL_NAMES`, currently `parse_workout`) are excluded so external clients can't drive our model spend. The Strava access token is resolved lazily, only when a strava-backed tool is actually called.
